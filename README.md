...
- The **Draw Tower** button spends **100 supplies** and adds one random tower to the inventory.
- The draw button now spins a visible dice cube through faces 1 to 6 before revealing the tower result and prevents duplicate clicks while the roll is in progress.
...
- Six regular enemy types and three elite types now rotate through the waves, with distinct health, speed, size, and color profiles.
...
- A monster portal sits near the beginning of every map path. It glows, rotates, and emits particles while a wave is active.
- Maps unlock strictly by stage through the defeat upgrade screen: Stage 1 Verdant Coast is available from the start, then **Unlock Stage 2** opens Ember Canyon, followed by **Unlock Stage 3** for Frostwind Pass. Changing maps returns placed towers to storage while preserving their levels.
...
- Use **Save Game** between waves to store progress in the current browser. Saved maps, unlocks, base upgrades, towers, inventory, resources, and camera position are restored when the game is opened again.
