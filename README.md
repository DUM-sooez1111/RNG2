...
- Every enemy displays its name and level above the health bar. Enemy levels rise every three waves and every 30 cleared waves add +1 level to all future enemies; higher map stages plus elite and boss status add further levels, health, and keep damage. Level 2 adds an aura, level 4 armor, level 6 spikes, and level 9 a crown.
...
- Every 30th wave (30, 60, 90, and so on) introduces one of six rotating bosses: Ancient Bastion, Inferno Sovereign, Tide Leviathan, Crystal Cataclysm, Dread Citadel, or Solar Colossus. A 2-second boss-introduction cutscene plays before the battle begins. Bosses have scaling health pools and a 500-supply reward. Every boss defeat permanently raises Permanent Keep Health by 1 level (+5 starting health) with no level cap, and immediately restores 5 keep health. It also has a 50% chance to increase permanent Roulette Mastery by one level (up to level 60).
- Every boss also drops 1 gem, plus 1 more per Emerald Bounty level. Open **GEM FORGE** to spend gems on permanent Ruby Strike tower damage, Sapphire Keep starting health, Emerald Bounty boss-gem upgrades, and sequential next-map unlocks. Gems and their upgrades are retained after defeat and map changes.
...
- Maps unlock strictly by stage through the Gem Forge: Stage 1 Verdant Coast is available from the start; every next-map unlock costs 5 gems and opens the next grade group. Each map keeps its own placed tower layout and tower levels when you switch away and return. Permanent upgrades and map unlocks are also saved independently, so changing maps never removes them.
- The battlefield and every route are 20% larger, with five more default build zones and four more build-slot upgrade zones per map. Reset View starts farther out, and the mouse wheel can zoom out further.
