...
- Maps unlock strictly by stage through the Gem Forge: Stage 1 Verdant Coast is available from the start; spend gems on the next map card to open Ember Canyon plus Mythic and Ancient grades, then Frostwind Pass plus Celestial through Transcendent grades, Thundercrown Ridge plus Eternal, and Obsidian Eclipse plus Apex. Each map keeps its own placed tower layout and tower levels when you switch away and return. Permanent upgrades and map unlocks are also saved independently, so changing maps never removes them.
