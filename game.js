...
const ui = {
  ...
  inventory: document.querySelector('#inventory'), towerCategories: document.querySelector('#towerCategories'), upgradePanel: document.querySelector('#upgradePanel'), resetViewButton: document.querySelector('#resetViewButton'), mapButton: document.querySelector('#mapButton'), previousMapButton: document.querySelector('#previousMapButton'), saveButton: document.querySelector('#saveButton'), indexButton: document.querySelector('#indexButton'), indexPanel: document.querySelector('#indexPanel'), statsButton: document.querySelector('#statsButton'), statsPanel: document.querySelector('#statsPanel'), gemButton: document.querySelector('#gemButton'), gemPanel: document.querySelector('#gemPanel'), guideButton: document.querySelector('#guideButton'), guidePanel: document.querySelector('#guidePanel'), drawCost: document.querySelector('#drawCost') || document.querySelector('#rollButton small'), rouletteLevel: document.querySelector('#rouletteLevel'), modal: document.querySelector('#modal'), modalTitle: document.querySelector('#modalTitle'), modalText: document.querySelector('#modalText'), baseUpgradeChoices: document.querySelector('#baseUpgradeChoices'), startButton: document.querySelector('#startButton')
};
...
let gems = 0;
let playSeconds = 0, lastPlaytimeSaveSecond = 0;
...
function loadPlaytimeRewards() { try { const saved=JSON.parse(localStorage.getItem(PLAYTIME_REWARD_KEY));playSeconds=Math.max(0,Math.floor(Number(saved?.seconds)||0));lastPlaytimeSaveSecond=Math.floor(playSeconds);if(Array.isArray(saved?.claimed))saved.claimed.forEach(id=>claimedPlaytimeRewards.add(String(id))); } catch {} }
function persistPlaytimeRewards() { try { localStorage.setItem(PLAYTIME_REWARD_KEY,JSON.stringify({seconds:Math.floor(playSeconds),claimed:[...claimedPlaytimeRewards]})); } catch {} }
...
const playtimeRewards = [
  {id:'5m',seconds:5*60,icon:'CO',label:'300 SUPPLIES',gold:300},
  {id:'10m',seconds:10*60,icon:'CO',label:'1,000 SUPPLIES',gold:1000},
  {id:'15m',seconds:15*60,icon:'GM',label:'10 GEMS',gems:10},
  {id:'20m',seconds:20*60,icon:'WALL',label:'EPIC BARRICADE',towerId:'epic-barricade'},
  {id:'30m',seconds:30*60,icon:'GM',label:'50 GEMS',gems:50},
  {id:'50m',seconds:50*60,icon:'TWR',label:'LEGENDARY TOWER',grade:'legendary'},
  {id:'60m',seconds:60*60,icon:'APX',label:'APEX TOWER',grade:'apex'}
];
...
function renderPlaytimeRewards() { const next=playtimeRewards.find(reward=>!claimedPlaytimeRewards.has(reward.id)),nextText=next?`NEXT ${Math.max(0,Math.ceil(next.seconds-playSeconds))}s`:'ALL CLAIMED';return `<h3 class="gem-section-title">PLAYTIME REWARDS</h3><p>Active play time: <b data-playtime-current>${formatPlaytime(playSeconds)}</b> | ${nextText}</p><div class="gem-upgrades playtime-rewards">${playtimeRewards.map(reward=>{const claimed=claimedPlaytimeRewards.has(reward.id),ready=playSeconds>=reward.seconds,status=claimed?'CLAIMED':ready?'CLAIM NOW':`${formatPlaytime(reward.seconds-playSeconds)} LEFT`;return `<button class="${ready&&!claimed?'ready':''}" data-playtime-claim="${reward.id}" ${claimed||!ready?'disabled':''}><span class="gem-upgrade-icon">${reward.icon}</span><span class="gem-upgrade-name">${formatPlaytime(reward.seconds)}</span><span class="gem-upgrade-detail">${reward.label}</span><span class="gem-upgrade-level">${status}</span></button>`;}).join('')}</div>`; }
...
function claimPlaytimeReward(id) { const reward=playtimeRewards.find(entry=>entry.id===id);if(!reward)return;if(claimedPlaytimeRewards.has(id))return say('This playtime reward has already been claimed.');if(playSeconds<reward.seconds)return say(`${formatPlaytime(reward.seconds-playSeconds)} more active play time is needed.`);let label=reward.label;if(reward.gold)gold+=reward.gold;if(reward.gems)gems+=reward.gems;if(reward.towerId&&types[reward.towerId]){inventory[reward.towerId].push(1);selected=reward.towerId;}if(reward.grade){const candidates=Object.values(types).filter(type=>type.grade===reward.grade),tower=candidates[Math.floor(Math.random()*candidates.length)];if(tower){inventory[tower.id].push(1);selected=tower.id;label=`${tower.name} (LV 1)`;}}claimedPlaytimeRewards.add(id);persistPlaytimeRewards();persistPermanentProgress();updateUI();renderGemPanel();say(`Playtime reward claimed: ${label}.`); }
