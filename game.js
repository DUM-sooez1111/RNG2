...
let towers = [], enemies = [], shots = [], particles = [], airstrikes = [], spawnQueue = [], spawnCooldown = 0, selectedTower = null, fusionMode = false, isDrawing = false, drawTimer, drawSpinTimer, rouletteHideTimer, autoWave = false, autoWaveTimer, autoDraw = false, autoDrawTimer, autoSaveTimer, collectAllArmed = false, activeSkill = null, airstrikeReadyAt = 0, bossIntro = null, tutorialStep = 0, tutorialActive = true;
...
function showRouletteScene() { clearTimeout(rouletteHideTimer);if(!ui.rouletteScene)return;ui.rouletteScene.hidden=false;ui.rouletteScene.classList.add('open'); }
function hideRouletteScene() { if(!ui.rouletteScene)return;ui.rouletteScene.classList.remove('open');ui.rouletteScene.hidden=true; }
...
if(ui.autoDrawButton) ui.autoDrawButton.addEventListener('click', () => { autoDraw=!autoDraw; updateAutoDrawButton(); say(`자동 뽑기 ${autoDraw?'켜짐':'꺼짐'}.`); if(autoDraw) queueAutoDraw(); else clearTimeout(autoDrawTimer); });
if(ui.rouletteAutoButton) ui.rouletteAutoButton.addEventListener('click', () => { autoDraw=!autoDraw;updateAutoDrawButton();say(`자동 뽑기 ${autoDraw?'켜짐':'꺼짐'}.`);if(autoDraw)queueAutoDraw();else clearTimeout(autoDrawTimer); });
if(ui.rouletteHideButton) ui.rouletteHideButton.addEventListener('click', hideRouletteScene);
