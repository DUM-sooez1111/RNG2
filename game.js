...
let towerCategory = 'all', towerGrade = 'all';
...
function renderIndex() { if(!ui.indexPanel) return; const towerCards=Object.values(types).map(type=>`<article class="index-card"><span class="index-icon ${type.kind}" style="--index-color:${type.color}">${type.icon}</span><span><b>${type.name}</b><small>${getAbilityText({type,level:1})}</small></span></article>`).join(''); const enemyCards=[...enemyTypes.map(type=>({...type,group:'NORMAL'})),...eliteTypes.map(type=>({...type,group:'ELITE'})),{...wave30Boss,group:'BOSS'}].map(type=>{const mark=type.name.split(' ').map(word=>word[0]).join('').slice(0,3);return `<article class="index-card enemy"><span class="index-enemy-avatar ${type.group.toLowerCase()}" style="--enemy-body:${type.body};--enemy-top:${type.top}"><i>${mark}</i></span><span><b>${type.name} <em>${type.group}</em></b><small>KEEP DMG ${type.damage} | SPD ${type.speed}</small></span></article>`;}).join(''); ui.indexPanel.innerHTML=`<div class="index-head"><strong>GAME INDEX</strong><button data-close-index>CLOSE</button></div><div class="index-scroll"><h3>TOWERS · ${Object.keys(types).length}</h3><div class="index-grid">${towerCards}</div><h3>ENEMIES · ${enemyTypes.length+eliteTypes.length+1}</h3><div class="index-grid">${enemyCards}</div></div>`; }
...
function updateUI() { const missingSupplies=Math.max(0,drawCost-gold), rouletteLevel=getRouletteLevel(); ui.gold.textContent = gold; ui.lives.textContent = lives; ui.wave.textContent = wave; if(ui.drawCost) ui.drawCost.textContent = drawCost; if(ui.rouletteLevel) ui.rouletteLevel.textContent=`LV ${rouletteLevel}`; ui.rollButton.title=missingSupplies ? `Tower roulette costs ${drawCost}. Need ${missingSupplies} more supplies.` : rouletteLevel<10 ? `Draw a random tower for ${drawCost} supplies. Epic and Legendary unlock at roulette level 10.` : rouletteLevel<20 ? `Draw a random tower for ${drawCost} supplies. Mythic and Ancient unlock at roulette level 20.` : rouletteLevel<40 ? `Draw a random tower for ${drawCost} supplies. Celestial and higher grades unlock at roulette level 40.` : `Draw a random tower for ${drawCost} supplies`; ui.rollButton.classList.toggle('needs-supplies',!!missingSupplies&&!isDrawing); ui.difficulty.textContent=`S${getStageDifficulty()} ${getDifficultyLabel()}`; renderInventory(); renderUpgradePanel(); updateMapButton(); updateAutoWaveButton(); if(ui.statsPanel&&!ui.statsPanel.hidden)renderStats(); }
...
function getMapGradeCap() { return unlockedMaps[2]?grades.length-1:unlockedMaps[1]?6:4; }
function getRouletteGrades() { const level=getRouletteLevel(); return grades.filter((grade,index)=>index<=getMapGradeCap()&&(level>=10||index<=2)&&(level>=20||index<=4)&&(level>=40||index<=6)); }
