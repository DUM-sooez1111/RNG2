...
function renderUpgradePanel() {
  if(!selectedTower || !towers.includes(selectedTower)) { ui.upgradePanel.className='upgrade-panel empty'; ui.upgradePanel.innerHTML=`<span class="upgrade-title">UPGRADE</span><span class="upgrade-detail">Click an installed tower.</span>${renderUpgradeAllButton()}`; return; }
...
ui.upgradePanel.addEventListener('click', event => { if(event.target.closest('[data-upgrade]')) upgradeTower(); if(event.target.closest('[data-upgrade-all]')) upgradeAllTowers(); if(event.target.closest('[data-fuse]')) startFusion(); if(event.target.closest('[data-cancel-fuse]')) cancelFusion(); if(event.target.closest('[data-store]')) storeTower(); });
