...
if(ui.gemPanel) ui.gemPanel.addEventListener('click', event => { if(event.target.closest('[data-close-gems]')){ui.gemPanel.hidden=true;return;} const gemButton=event.target.closest('[data-gem-upgrade]'); if(gemButton) buyGemUpgrade(gemButton.dataset.gemUpgrade); const baseButton=event.target.closest('[data-base-gem-upgrade]'); if(baseButton) buyBaseGemUpgrade(baseButton.dataset.baseGemUpgrade); if(event.target.closest('[data-map-gem-unlock]')) buyNextMapWithGems(); const skillPurchase=event.target.closest('[data-buy-skill]'); if(skillPurchase) buyPlayerSkill(skillPurchase.dataset.buySkill); const skillEquip=event.target.closest('[data-equip-skill]'); if(skillEquip) equipPlayerSkill(skillEquip.dataset.equipSkill); });
