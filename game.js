...
function renderInventory() {
  ...
  if(ui.towerCategories)ui.towerCategories.innerHTML=`<div class="tower-kind-categories">${categories.map(([id,label])=>`<button data-category="${id}" class="${towerCategory===id?'active':''}" title="Show ${label.toLowerCase()} towers">${label}</button>`).join('')}</div><div class="tower-grade-categories"><button data-grade="all" class="${towerGrade==='all'?'active':''}">ALL GRADES</button>${grades.map(grade=>`<button data-grade="${grade.id}" class="grade-${grade.id} ${towerGrade===grade.id?'active':''}">${grade.label}</button>`).join('')}</div>`;
