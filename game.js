...
let towers = [], enemies = [], shots = [], particles = [], spawnQueue = [], spawnCooldown = 0, selectedTower = null, fusionMode = false, isDrawing = false, drawTimer, drawSpinTimer, autoWave = false, autoWaveTimer, autoDraw = false, autoDrawTimer, tutorialStep = 0, tutorialActive = true;
...
const tutorialPages = [
  ...
  {title:'FIELD MANUAL 3 / 3', text:'Upgrade and fuse matching towers for stronger grades. Bosses drop gems; from Stage 2 onward, gems can also improve your permanent base upgrades. Use the Guidebook button whenever you need a refresher.'}
];
...
function serializeEnemy(enemy) { return {typeName:enemy.type.name,progress:enemy.progress,spawnProgress:enemy.spawnProgress,speed:enemy.speed,hp:enemy.hp,maxHp:enemy.maxHp,damage:enemy.damage,reward:enemy.reward,slow:enemy.slow,slowFactor:enemy.slowFactor,poison:enemy.poison,poisonDamage:enemy.poisonDamage,burn:enemy.burn,burnDamage:enemy.burnDamage,statusTick:enemy.statusTick,hit:enemy.hit}; }
function restoreSavedEnemy(saved) { const type=[...enemyTypes,...eliteTypes,wave30Boss].find(entry=>entry.name===saved.typeName); if(!type||!Number.isFinite(saved.progress)||!Number.isFinite(saved.hp)) return null; const boss=type===wave30Boss,elite=eliteTypes.includes(type),progress=Math.max(PORTAL_PROGRESS,saved.progress),spawnProgress=Math.min(progress,Math.max(0,Number.isFinite(saved.spawnProgress)?saved.spawnProgress:PORTAL_PROGRESS)),maxHp=Math.max(1,Math.floor(saved.maxHp||1)),enemy={progress,spawnProgress,speed:Math.max(1,Number(saved.speed)||1),hp:Math.max(1,Math.min(maxHp,Number(saved.hp))),maxHp,damage:Math.max(1,Math.floor(saved.damage||type.damage)),reward:Math.max(0,Math.floor(saved.reward||0)),r:type.r,elite,boss,type,slow:Math.max(0,Number(saved.slow)||0),slowFactor:Math.max(.25,Math.min(1,Number(saved.slowFactor)||1)),poison:Math.max(0,Number(saved.poison)||0),poisonDamage:Math.max(0,Number(saved.poisonDamage)||0),burn:Math.max(0,Number(saved.burn)||0),burnDamage:Math.max(0,Number(saved.burnDamage)||0),statusTick:Math.max(0,Number(saved.statusTick)||0),hit:Math.max(0,Number(saved.hit)||0)}; const pos=pointOnPath(progress); return {...enemy,x:pos.x,y:pos.y,angle:pos.angle}; }
function saveGame() { try { captureCurrentMapLayout(); persistPermanentProgress(); const state={version:1,mapIndex:currentMapIndex,unlockedMaps:[...unlockedMaps],baseUpgrades:{...baseUpgrades},gold,lives,wave,drawCost,running,autoWave,spawnQueue:[...spawnQueue],spawnCooldown,enemies:enemies.map(serializeEnemy),inventory,mapTowerStates:mapTowerStates.map(layout=>layout.map(tower=>({...tower}))),towers:mapTowerStates[currentMapIndex],selected,camera:{...camera}}; localStorage.setItem(SAVE_KEY,JSON.stringify(state)); say(running?'Wave progress saved in this browser.':'Game saved in this browser.'); } catch { say('Unable to save in this browser.'); } }
...
function getIndexEnemies() { return [...enemyTypes.map((type,index)=>({...type,group:'NORMAL',id:`normal-${index}`})),...eliteTypes.map((type,index)=>({...type,group:'ELITE',id:`elite-${index}`})),{...wave30Boss,group:'BOSS',id:'boss-0'}]; }
...
function canUseGemBaseUpgrades() { return currentMapIndex>=1; }
...
function getUpgradeableTowers() { return towers.filter(tower=>tower.level<getMaxLevel(tower)); }
function getUpgradeAllCost(targets=getUpgradeableTowers()) { return targets.reduce((total,tower)=>total+upgradeCost(tower),0); }
function renderUpgradeAllButton() { const targets=getUpgradeableTowers(), cost=getUpgradeAllCost(targets), disabled=fusionMode||!targets.length||gold<cost; return `<button class="upgrade-all" data-upgrade-all ${disabled?'disabled':''}>ALL UPGRADE ${targets.length ? `${targets.length} • ${cost}` : 'MAXED'}</button>`; }
function renderCollectAllButton() { return `<button class="collect-all" data-collect-all ${towers.length?'':'disabled'}>COLLECT ALL${towers.length?` ${towers.length}`:''}</button>`; }
function renderUpgradePanel() {
  if(!selectedTower || !towers.includes(selectedTower)) { ui.upgradePanel.className='upgrade-panel empty'; ui.upgradePanel.innerHTML=`<span class="upgrade-title">UPGRADE</span><span class="upgrade-detail">Click an installed tower.</span>${renderUpgradeAllButton()}${renderCollectAllButton()}`; return; }
...
function makeEnemy(index) { const stage=getStageDifficulty(), boss = wave > 0 && wave % 30 === 0 && index === 0, elite = !boss && wave % 4 === 0 && index === 0; const type=boss ? wave30Boss : elite ? eliteTypes[Math.floor(wave/4)%eliteTypes.length] : enemyTypes[(wave*3+index)%enemyTypes.length], waveScale=Math.max(0,wave-1), healthScale=1+waveScale*.03, damageScale=1+waveScale*.03; const baseHp=(boss ? 5400+wave*100 : elite ? 310+wave*50 : 58+wave*24)*(1+(stage-1)*.4), hp=Math.round(baseHp*type.hp*healthScale), damage=Math.max(1,Math.ceil((type.damage+stage-1)*damageScale)), reward=(boss ? 500 : elite ? 25 : 10)+waveScale*5; return {progress:PORTAL_PROGRESS, spawnProgress:PORTAL_PROGRESS, speed:(38+wave*3.4)*type.speed*(1+(stage-1)*.1), hp, maxHp:hp, damage, reward, r:type.r, elite, boss, type, slow:0, slowFactor:1, poison:0, poisonDamage:0, burn:0, burnDamage:0, statusTick:0, hit:0}; }
...
function renderBaseUpgradeChoices() {
  ...
  if(nextMap!==-1){cards.push({id:'map',icon:'MAP',name:`UNLOCK STAGE ${maps[nextMap].stage}`,detail:`Unlock ${maps[nextMap].name} and ${mapUnlockGrades[nextMap]}`,mapIndex:nextMap});}
...
function reset() { clearTimeout(drawTimer); clearInterval(drawSpinTimer); clearTimeout(autoWaveTimer); clearTimeout(autoDrawTimer); autoWave=false; autoDraw=false; isDrawing=false; ui.rollButton.disabled=false; ui.rollButton.classList.remove('rolling'); setRouletteTower(); gold=320; lives=getStartingLives(); wave=0; drawCost=BASE_DRAW_COST; running=false; gameOver=false; towers=[]; mapTowerStates.forEach((_,index)=>mapTowerStates[index]=[]); enemies=[]; shots=[]; particles=[]; spawnQueue=[]; inventory=freshInventory(); selected='common-spark-coil'; selectedTower=null; fusionMode=false; resetView(); pads.forEach(p => delete p.tower); ui.waveButton.disabled=false; ui.waveButton.textContent='START WAVE >'; updateUI(); }
...
function upgradeAllTowers() { if(fusionMode) return say('Cancel fusion before upgrading all towers.'); const targets=getUpgradeableTowers(), cost=getUpgradeAllCost(targets); if(!targets.length) return say('All installed towers are already at max level.'); if(gold<cost) return say(`You need ${cost-gold} more supplies to upgrade all ${targets.length} towers.`); gold-=cost; targets.forEach(tower=>{tower.level++;tower.pulse=.28;}); updateUI(); say(`${targets.length} towers upgraded by 1 level! Supplies -${cost}.`); }
...
ui.upgradePanel.addEventListener('click', event => { if(event.target.closest('[data-upgrade]')) upgradeTower(); if(event.target.closest('[data-upgrade-all]')) upgradeAllTowers(); if(event.target.closest('[data-fuse]')) startFusion(); if(event.target.closest('[data-cancel-fuse]')) cancelFusion(); if(event.target.closest('[data-store]')) storeTower(); if(event.target.closest('[data-collect-all]')) collectAllTowers(); });
...
if(ui.gemPanel) ui.gemPanel.addEventListener('click', event => { if(event.target.closest('[data-close-gems]')){ui.gemPanel.hidden=true;return;} const gemButton=event.target.closest('[data-gem-upgrade]'); if(gemButton) buyGemUpgrade(gemButton.dataset.gemUpgrade); const baseButton=event.target.closest('[data-base-gem-upgrade]'); if(baseButton) buyBaseGemUpgrade(baseButton.dataset.baseGemUpgrade); });
...
window.addEventListener('keydown', e => { const key=e.key.toLowerCase(); if(['w','a','s','d'].includes(key)){cameraKeys.add(key);e.preventDefault();} if(e.key === ' '){ e.preventDefault(); if(!ui.modal.classList.contains('open')) startWave(); } if(['1','2','3'].includes(e.key)&&!ui.modal.classList.contains('open')) choose(['common-spark-coil','common-root-cannon','common-thorn-garden'][+e.key-1]); });
...
function applyThornAbility(tower, enemy) { const tier=getGradeTier(tower); if(tower.type.ability==='root') applySlow(enemy,.35+tier*.1,.74-tier*.035); if(tower.type.ability==='freeze') applySlow(enemy,.7+tier*.12,.54-tier*.035); if(tower.type.ability==='poison') applyDot(enemy,'poison',2+tier*.35,getTowerDamage(tower)*(.16+tier*.035)); }
...
function update(dt) {
  ...
  for(const enemy of [...enemies]) { enemy.poison=Math.max(0,(enemy.poison||0)-dt); enemy.burn=Math.max(0,(enemy.burn||0)-dt); enemy.statusTick=(enemy.statusTick||0)-dt; if(enemy.statusTick<=0&&(enemy.poison>0||enemy.burn>0)){enemy.statusTick=.35;const dotDamage=(enemy.poison>0?enemy.poisonDamage:0)+(enemy.burn>0?enemy.burnDamage:0);hurt(enemy,dotDamage*.35,enemy.burn>0?'#ff9d55':'#ae7cff');if(!enemies.includes(enemy))continue;} enemy.progress=Math.max(enemy.spawnProgress,enemy.progress+enemy.speed*dt*(enemy.slow > 0 ? enemy.slowFactor||.62 : 1)); enemy.slow=Math.max(0,enemy.slow-dt); if(!enemy.slow) enemy.slowFactor=1; enemy.hit=Math.max(0,enemy.hit-dt); const pos=pointOnPath(enemy.progress); enemy.x=pos.x; enemy.y=pos.y; enemy.angle=pos.angle; if(enemy.progress >= pathLength){ enemies.splice(enemies.indexOf(enemy),1); lives=Math.max(0,lives-enemy.damage); updateUI(); say(`${enemy.type.name} hit the keep for ${enemy.damage} damage!`); if(lives <= 0){ gameOver=true; running=false; recordStats({deaths:1}); showDeathUpgrades(); } } }
  for(const tower of towers) { tower.cooldown -= dt; tower.pulse=Math.max(0,tower.pulse-dt); const range=getTowerRange(tower), targets=enemies.filter(e=>distance(tower,e)<range); if(!targets.length) continue; const target=targets.reduce((best,e)=>e.progress>best.progress?e:best,targets[0]); tower.angle=Math.atan2(target.y-tower.y,target.x-tower.x); if(tower.type.kind === 'thorns'){ if(tower.cooldown <= 0){ targets.filter(e=>distance(tower,e)<range).forEach(enemy=>{applyThornAbility(tower,enemy);hurt(enemy,getTowerDamage(tower),tower.type.color);}); tower.cooldown=getTowerFireRate(tower); tower.pulse=.25; } } else if(tower.cooldown <= 0){ tower.cooldown=getTowerFireRate(tower); tower.pulse=.15; shots.push({x:tower.x,y:tower.y,target,tower,damage:getTowerDamage(tower),color:tower.type.color,kind:tower.type.kind,speed:tower.type.kind === 'spark' ? 720 : 410}); } }
  for(const shot of [...shots]) { if(!enemies.includes(shot.target)){ shots.splice(shots.indexOf(shot),1); continue; } const dx=shot.target.x-shot.x,dy=shot.target.y-shot.y,d=Math.hypot(dx,dy),move=shot.speed*dt; if(d < move+shot.target.r){ hurt(shot.target,shot.damage,shot.color); triggerShotAbility(shot); shots.splice(shots.indexOf(shot),1); } else { shot.x+=dx/d*move; shot.y+=dy/d*move; } }
...
function drawGradeEffect(t) {
  ...
  if(tier>=1){ctx.globalAlpha=.16+.05*Math.sin(time*3+t.x);ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,27+Math.sin(time*3+t.y)*2,0,Math.PI*2);ctx.stroke();}
...
function drawFusionReadyEffect(t) {
  ...
  const time=performance.now()/1000, radius=34+Math.sin(time*5+t.x*.02)*3;
  ctx.save();
  ctx.globalCompositeOperation='lighter';
  ctx.globalAlpha=.2+.12*Math.sin(time*5+t.y*.02);
